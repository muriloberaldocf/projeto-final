...
echo "<h3>🚀 DOBRANDO O BANCO PARA 240.000 QUESTÕES (2.000 por Tópico)...</h3>";
...
foreach ($structure as $subjectId => $unitsList) {
    foreach ($unitsList as $uIndex => $unitInfo) {
        ...
        foreach ($unitInfo['lessons'] as $topicTitle) {
            ...
            // DOBRO DE QUESTÕES: Gerar 2.000 QUESTÕES por tópico (120 tópicos * 2.000 = 240.000 questões!)
            for ($qNum = 1; $qNum <= 2000; $qNum++) {
...
echo "<div style='font-family:sans-serif; padding:20px; background:#eef2ff; color:#3730a3; border-radius:10px; margin:20px;'>
    <h2>🎉 O DOBRO ALCANÇADO: 240.000 QUESTÕES POPULADAS!</h2>
    <p>Foram populadas <strong>" . number_format($totalQuestions, 0, ',', '.') . " questões ativas</strong> (2.000 em cada um dos 120 Tópicos)!</p>
</div>";
